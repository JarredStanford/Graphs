...
for room in roomGraph:
    ...
    for direction, target in roomGraph[room][1].items():
        #print(direction, target)
        graph.vertices[room][direction] = target
        #print(graph.vertices[room][direction])

def bft(starting_vertex, direction = None):
    """
    ...
    q = Queue()
    q.enqueue([direction, starting_vertex])
    ...
    while q.size() > 0:
        ...
        if path[0] is not None:
            traversalPath.append(path[0])
        v = path[1]
        # If that vertex has not been visited...
        if v not in visited:
            # Mark it as visited
            visited[v] = {'e': 0, 'n': 0, 's': 0, 'w': 0, 'prev': [-1, -1]}
            visited[v]['prev'] = [last_direction, prev]
            visited_rooms.add(v)
        # Then add all of its neighbors to the back of the queue
        for direction, neighbor in graph.vertices[v].items():
            if neighbor != '?' and visited[v][direction] == 0 and neighbor != prev:
                visited[v][direction] = 1
                last_direction = opposite[direction]
                prev = v
                if neighbor not in visited:
                    visited[neighbor] = {'n': 0, 's': 0, 'w': 0, 'e': 0, 'prev': [last_direction, prev]}
                visited[neighbor][last_direction] = 1
                visited_rooms.add(neighbor)
                if visited[v]['prev'][1] == -1:
                    visited[v]['prev'] = ['w', 3]
                q.enqueue([direction, neighbor])
                break
        while q.size() == 0 and len(visited_rooms) != len(roomGraph):
            ...
            checked = set()
            steps = []
            while newQ.size() is not 0 and q.size() is 0:
                new_path = newQ.dequeue()
                x = new_path[-1][1]
                print(x, len(visited_rooms))
                if x not in checked:
                    checked.add(x)
                    if x not in visited_rooms:
                        for array in new_path:
                            steps.append(array[0])
                        steps.pop(0)
                        traversalPath += steps
                        q.enqueue([None, x])
                    else:
                        for direction, neighbor in graph.vertices[x].items():
                            new_path_copy = new_path.copy()
                            new_path_copy.append([direction, neighbor])
                            newQ.enqueue(new_path_copy)
